...
public class Juego extends InterfaceJuego {
	...
	public void tick() {
        ...
        // Ponemos el pubtaje y los enemgios eliminados.
		entorno.cambiarFont("Consolas", 12, Color.WHITE);  							 // Utilizamos el metodo ".cambiarFont" para cambiar la fuente del texto que se va a mostrar en pantalla del juego.
        entorno.escribirTexto("Enemigos eliminados: " + enemigosEliminados, 5, 15); // Luego llamamos al metodo ".escribirTexto" para dibujar el texto deseado en la pantalla del juego. Con este metodo podemos elegir donde queremos que se posicione.
	    ...
	    if (enemigos.isEmpty()) {
	    	generarEnemigos(8); // Genera nuevos enemigos.
	    }
	}
	...
	public boolean detectarColision(Shooting disparo, Monstruo enemigo) {
	    // Verificar si la bala y el enemigo se superponen
	...
	public boolean detectarColision(Monstruo m, Piso so) {
		for (int i = 0; i < so.piso.length; i++) {
			if(so.piso[i] != null && detectarColision(m, so.piso[i])) {
				return true;
			}							// Aca sacamos el if de irrompible, porque
		}								// sino los enemigos rompen el piso
		return false;
	}
